...
class SaleWizard(models.TransientModel):
    _name = 'odoo_training.sale.wizard'
    _description = "Wizard: Quick Sale Orders for Session Students"

    def _default_session(self):
        return self.env['odoo_training.session'].browse(self._context.get('active_id'))
    
    session_id = fields.Many2one(comodel_name="odoo_training.session", string='Session ', required=True, default=_default_session)
